//Project 5 ~ Noah Jones (nhj0004)
//COMP2710 ~ Software Construction ~ Dr. Li
//Compiled using: the command 'g++ Project5_Jones_nhj0004.cpp -o "p4" ' and then './p4' on a linux machine
...
// We need the mutex to make sure that only one thread at a time can access the buffer
// being the consumer thread or the producer thread.
//Declare Mutex
pthread_mutex_t the_mutex;

// A blocked thread waits for the "traffic lights" before it make an action.
// The condition varibles are the "traffic lights" to tell the threads
// whether the job is done and ~ if the resource, the buffer in this case, is available 
...
void *producer(void *ptr)
{
    ...
    for(i=1; i<=MAX; i++)
    {
        ...
        // Step 4, print out message telling what item the producer generated.
